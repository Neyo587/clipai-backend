...
const BYPASS = `--extractor-args "youtube:skip=dash,hls;player_client=ios,web" --no-warnings`;
function proxyArg() {
...
app.post('/api/info', (req, res) => {
  ...
  const cmd = `"${YTDLP}" --cookies "/tmp/yt-cookies.txt" --proxy "${process.env.PROXY_URL || ''}" --extractor-args "youtube:skip=dash,hls;player_client=ios,web" --no-playlist --print "%(title)s|||%(duration_string)s|||%(id)s" "${url}"`;
